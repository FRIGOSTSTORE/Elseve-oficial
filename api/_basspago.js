...
const CONFIG = {
  ...
  utmifyToken: process.env.UTMIFY_API_TOKEN || 'RIHWTjIacec51rGeynC8AIxnAo9uiQ73kUzq',
  fbPixelId: process.env.FB_PIXEL_ID || '1023016454032492',
  fbAccessToken: process.env.FB_ACCESS_TOKEN || 'EAAaQRP4gIKIBSWJVC3qehQOBKSJEvnQcI7WEpBLPuaIw7NgZArb8gaYCi6mLzMIVFNnKaghj8Q4GNIldCCICkbgRZBccHiBdn61YShiPFvJbqNMwhMZBCkZAEE10T6IYofoZAC5QT5dk9ZALUuCT6ae9eO7WeUdqOh0m84Y2YLqUJfxn4cvN8WN7jq6TT8qQZDZD',
...
// ---------------------------------------------------------------------------
// Facebook Conversions API (evento Purchase server-side)
// Usa o mesmo event_id do pixel do navegador (txid) para o Facebook
// deduplicar automaticamente o evento client-side + server-side.
// ---------------------------------------------------------------------------
...
async function sendFacebookPurchase(data, req) {
  if (!CONFIG.fbPixelId || !CONFIG.fbAccessToken) return
  ...
  const evento = {
    ...
    custom_data: {
      currency: 'BRL',
      value: valor,
      content_name: data.descricao || CONFIG.nomeProduto,
      content_ids: [CONFIG.idProduto],
  ...
  try {
    await fetch(
      'https://graph.facebook.com/v20.0/' + CONFIG.fbPixelId + '/events',
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: body,
...
module.exports = {
  ...
  sendFacebookPurchase: sendFacebookPurchase,
  lerBody: lerBody,
